...
public class BladeModeScript : MonoBehaviour
{
    ...
    public void Zoom(bool state)   // this is fucking god code !!!!
    {
        bladeMode = state;
        anim.SetBool("bladeMode", bladeMode);

        cutPlane.localEulerAngles = Vector3.zero;
        cutPlane.gameObject.SetActive(state);   // enable the cutPlane 

        string x = state ? "Horizontal" : "Mouse X"; string y = state ? "Vertical" : "Mouse Y";  
        TPCamera.m_XAxis.m_InputAxisName = x; TPCamera.m_YAxis.m_InputAxisName = y;   // this selects the input axis name 

        float fov = state ? zoomFOV : normalFOV;
        Vector3 offset = state ? zoomOffset : normalOffset;
        float timeScale = state ? .2f : 1;

        DOVirtual.Float(Time.timeScale, timeScale, .02f, SetTimeScale);  // slow motion...
        DOVirtual.Float(TPCamera.m_Lens.FieldOfView, fov, .1f, FieldOfView);  // change in FOV
        ...
        {
            transform.DORotate(new Vector3(0, transform.eulerAngles.y, 0), .2f);  // rotate the player towards look direction....
        }
